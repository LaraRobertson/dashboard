 ...
 * })
 */
export default (data, data2, loggingEnabled = false): DataProvider => {
    ...
    function getResponse(type, resource, params, data, data2) {
        ...
        if (resource === "endpoint") {
            console.log ("resource is endpoint");
            restServer.init(data2);
        } else if (resource === "users") {
            console.log("run data2 for users");
            restServer.init(data2);
        } else {
    ...
    const handle = (type, resource, params): Promise<any> => {
        ...
        console.log("data2-handle2", data2);
        console.log("resource: " + resource);
        const restServer = new FakeRest.Server();
        ...
        let response;
        try {
            response = getResponse(type, resource, params, data, data2);
        } catch (error) {
            console.error(error);
            return Promise.reject(error);
        }
    ...
    return {
        getList: (resource, params) => handle('getList', resource, params, data, data2),
